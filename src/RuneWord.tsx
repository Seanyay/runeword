...
function RuneWord(props: IProps) {
    ...
    return (
      <>
        <div className="WordPart">
          <img key={r?.id} className="SmallRune" src={r?.image} alt={`${r?.name}`} title={`${r?.name}`} />
          <div>{r?.name}</div>
        </div>
  ...
  return (
    <div className="RuneWord">
      ...
      <div className="Type"><span className="Sockets">({runes.length}) Socket</span> {itemTypes.join(', ')}</div>
